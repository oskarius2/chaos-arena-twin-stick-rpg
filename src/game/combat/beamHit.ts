...
export function applyBeamHit(ctx: BeamHitContext, e: Entity): void {
  ...
  if (Math.random() > 1 - 0.5 / chaosFactor) {
    next.particles.push(...createImpact(e.pos, '#00f2ff', isCrit ? 6 : 3));
  }
...
function handleEnemyDeath(ctx: BeamHitContext, e: Entity, chaosFactor: number): void {
  ...
  const explCount = e.enemyType === EnemyType.BOSS ? 100 : Math.max(5, 20 / chaosFactor);
  next.particles.push(...createExplosion(e.pos, e.color, explCount, 1.5));

  const shakeMult = 1 / Math.sqrt(chaosFactor);
  next.screenshake = Math.min(
    next.screenshake + (e.enemyType === EnemyType.BOSS ? 20 : 8) * shakeMult,
    25
  );
