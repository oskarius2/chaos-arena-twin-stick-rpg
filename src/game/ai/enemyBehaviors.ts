...
export function runEnemyAttacks(
  ...
  if (enemy.enemyType === EnemyType.BOSS) {
    const baseInterval = slow ? 3000 : 1200;
    if (!fireCooldownReady(enemy, baseInterval, state)) return;
    markFired(enemy);
    const attackPattern = Math.floor(Date.now() / 1500) % 3;
    if (attackPattern === 0) {
      for (let a = 0; a < 24; a++) {
        const angle = (a / 24) * Math.PI * 2;
        pushProjectile(state, enemy, angle, 6, 14, '#ef4444', enemy.damage || 30);
      }
    } else {
      fireAtPlayer(state, enemy, angleToPlayer, {
        speed: 12,
  ...
  if (enemy.enemyType === EnemyType.CHASER || enemy.enemyType === EnemyType.SWARMER || !enemy.enemyType) {
    tryRetreatFire(state, enemy, dist, angleToPlayer, toPlayer, vx, vy, {
      ...
      color: enemy.enemyType === EnemyType.SWARMER ? '#fb923c' : '#f87171',
...
export function computeEnemyVelocity(
  ...
  switch (enemy.enemyType ?? EnemyType.CHASER) {
    ...
    default: {
      ...
      const burst = enemy.aiTimer <= 0 && dist > 100 && enemy.aiState !== 'retreat';
      if (burst) {
        enemy.aiState = 'lunge';
        enemy.aiTimer = 12;
      }
      if (enemy.aiState === 'retreat' && enemy.aiTimer > 0) {
        ...
        const speedMult = enemy.aiState === 'lunge' && enemy.aiTimer > 0 ? 1.6 : 1;
